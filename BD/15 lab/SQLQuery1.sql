...
<ДИСЦИПЛИНЫ>
<ДИСЦИПЛИНА КОД="КС" НАЗВАНИЕ="КСИС МИРОНОВ" КАФЕДРА="ИСИТ" />
<ДИСЦИПЛИНА КОД="ТРПОО" НАЗВАНИЕ="ТЕХНОЛОГИИ РАЗРАБОТКИ ПО" КАФЕДРА="ИСИТ" />
...
GO
SELECT * FROM STUDENT WHERE NAME = 'Дрозд А.И.'
GO
...
FROM  STUDENT WHERE NAME = 'Дрозд А.И.';  
...
GO
SELECT NAME, INFO FROM STUDENT WHERE NAME='Дрозд А.И.';
GO
